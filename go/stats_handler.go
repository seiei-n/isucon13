...
func getUserStatisticsHandler(c echo.Context) error {
	...
	if err := tx.SelectContext(ctx, &livecomments, `SELECT livecomments.Tip FROM livestreams LEFT JOIN livecomments ON livestreams.ID = livecomments.livestream_id WHERE livestreams.user_id = ?`, user.ID); err != nil && !errors.Is(err, sql.ErrNoRows) {
